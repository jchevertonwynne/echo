...
// echo has no HTML, so a browser falls back to /favicon.ico. Both paths must
// serve the icon, and neither may be swallowed by the echo handler.
func TestServesItsIcon(t *testing.T) {
	for _, path := range []string{"/icon.svg", "/favicon.ico"} {
		rec := httptest.NewRecorder()
		serveIcon(rec, httptest.NewRequest(http.MethodGet, path, nil))
		...
		if !strings.Contains(rec.Body.String(), "<svg") {
			t.Errorf("%s did not return an SVG", path)
		}
	}
}
